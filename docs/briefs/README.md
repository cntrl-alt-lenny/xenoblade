...
| 044 | [`scaffolder-scngroup-double-define-investigate`](044-scaffolder-scngroup-double-define-investigate.md) | Confirm + document the stale-asm artifact the decomper flagged in PR #44's notes (`ScnGroup` defined twice in `build/us/asm/`); add troubleshooting note to AGENTS.md. | Open |
| 045 | [`decomper-g3d-anmclr-flip-retry-2`](045-decomper-g3d-anmclr-flip-retry-2.md) | Retry the `g3d_anmclr.cpp` flip (NonMatching → Matching, US) now that brief 042 (PR #43) cleared the `.sdata 0x80663458` over-claim. | Open |
